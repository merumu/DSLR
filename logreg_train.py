...
def training(data):
    ...
    gryffindor = LogisticRegression(alpha=0.1, max_iter=1000, verbose=True, learning_rate='constant')
    ...
    ravenclaw = LogisticRegression(alpha=0.1, max_iter=1000, verbose=True, learning_rate='constant')
    y_train = np.where(y == 'Ravenclaw', 1, 0)
    ravenclaw.fit(x_train, y_train)
    print(ravenclaw.thetas)

    slytherin = LogisticRegression(alpha=0.1, max_iter=1000, verbose=True, learning_rate='constant')
    y_train = np.where(y == 'Slytherin', 1, 0)
    slytherin.fit(x_train, y_train)
    print(slytherin.thetas)

    hufflepuff = LogisticRegression(alpha=0.1, max_iter=1000, verbose=True, learning_rate='constant')
